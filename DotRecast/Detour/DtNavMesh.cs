     ...
     */
        ...
        List<long> QueryPolygonsInTile(DtMeshTile tile, RcVec3f qmin, RcVec3f qmax)
        {
            ...
            if (tile.data.bvTree != null)
            {
                ...
                float minx = MathExtension.Clamp(qmin.X, tbmin.X, tbmax.X) - tbmin.X;
                float miny = MathExtension.Clamp(qmin.Y, tbmin.Y, tbmax.Y) - tbmin.Y;
                float minz = MathExtension.Clamp(qmin.Z, tbmin.Z, tbmax.Z) - tbmin.Z;
                float maxx = MathExtension.Clamp(qmax.X, tbmin.X, tbmax.X) - tbmin.X;
                float maxy = MathExtension.Clamp(qmax.Y, tbmin.Y, tbmax.Y) - tbmin.Y;
                float maxz = MathExtension.Clamp(qmax.Z, tbmin.Z, tbmax.Z) - tbmin.Z;
                // Quantize
        ...
        public long AddTile(DtMeshData data, int flags, long lastRef)
        {
            ...
            for (int i = 0; i < tile.polyLinks.Length; i++)
            {
                tile.polyLinks[i] = DtNavMesh.DT_NULL_LINK;
            }
                ...
                for (int j = 0; j < nv; ++j)
                {
                    ...
                    foreach (var connectPoly in connectPolys)
                    {
                        ...
                        if (dir == 0 || dir == 4)
                        {
                            ...
                            link.bmin = (int)Math.Round(MathExtension.Clamp(tmin, 0.0f, 1.0f) * 255.0f);
                            link.bmax = (int)Math.Round(MathExtension.Clamp(tmax, 0.0f, 1.0f) * 255.0f);
                        }
                        else if (dir == 2 || dir == 6)
                        {
     ...
     */
        ...
        private long FindNearestPolyInTile(DtMeshTile tile, RcVec3f center, RcVec3f halfExtents, out RcVec3f nearestPt)
        {
            ...
            for (int i = 0; i < polys.Count; ++i)
            {
                ...
                if (posOverPoly)
                {
                    d = (float)Math.Abs(diff.Y) - tile.data.header.walkableClimb;
                    d = d > 0 ? d * d : 0;
                }
                else
                {
