        ...
        private static int CreateBVTree(DtNavMeshCreateParams option, DtBVNode[] nodes)
        {
            ...
            for (int i = 0; i < option.polyCount; i++)
            {
                ...
                if (option.detailMeshes != null)
                {
                    ...
                    it.bmin[0] = MathExtension.Clamp((int)((bmin.X - option.bmin.X) * quantFactor), 0, int.MaxValue);
                    it.bmin[1] = MathExtension.Clamp((int)((bmin.Y - option.bmin.Y) * quantFactor), 0, int.MaxValue);
                    it.bmin[2] = MathExtension.Clamp((int)((bmin.Z - option.bmin.Z) * quantFactor), 0, int.MaxValue);

                    it.bmax[0] = MathExtension.Clamp((int)((bmax.X - option.bmin.X) * quantFactor), 0, int.MaxValue);
                    it.bmax[1] = MathExtension.Clamp((int)((bmax.Y - option.bmin.Y) * quantFactor), 0, int.MaxValue);
                    it.bmax[2] = MathExtension.Clamp((int)((bmax.Z - option.bmin.Z) * quantFactor), 0, int.MaxValue);
                }
                else
                {
