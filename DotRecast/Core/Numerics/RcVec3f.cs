...
namespace DotRecast.Core.Numerics
{
    public struct RcVec3f
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public float Length() // readonly
        {
            float lengthSquared = LengthSquared(); 
            return (float)Math.Sqrt(lengthSquared); // MathF
        }
        ...
        public static float Distance(RcVec3f value1, RcVec3f value2)
        {
            ...
            return (float)Math.Sqrt(distanceSquared); // MathF
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void CopyTo(float[] array) // readonly
        {
            CopyTo(array, 0);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void CopyTo(float[] array, int n) // readonly
        {
            array[n + 0] = X;
            array[n + 1] = Y;
            array[n + 2] = Z;
        ...
        {
            float d = 1.0f / (float)Math.Sqrt(RcMath.Sqr(v.X) + RcMath.Sqr(v.Y) + RcMath.Sqr(v.Z)); // MathF
