            ...
            for (int columnIndex = 0; columnIndex < numColumns; ++columnIndex)
            {
                ...
                for (; span != null; span = span.next)
                {
                    if (span.area != RC_NULL_AREA)
                    {
                        ...
                        tempSpans[currentCellIndex].y = MathExtension.Clamp(bot, 0, MAX_HEIGHT);
                        tempSpans[currentCellIndex].h = MathExtension.Clamp(top - bot, 0, MAX_HEIGHT);
                    ...
                    for (int i = cell.index, ni = cell.index + cell.count; i < ni; ++i)
                    {
                        ...
                        for (int dir = 0; dir < 4; ++dir)
                        {
                            ...
                            for (int k = neighborCell.index, nk = neighborCell.index + neighborCell.count; k < nk; ++k)
                            {
                                ...
                                if ((top - bot) >= walkableHeight && (float)Math.Abs(neighborSpan.y - s.y) <= walkableClimb)
                                {
