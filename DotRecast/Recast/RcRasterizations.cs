...
namespace DotRecast.Recast
{
    public static class RcRasterizations
    {
            ...
            int flagMergeThreshold)
        {
            ...
            z0 = MathExtension.Clamp(z0, -1, h - 1);
            z1 = MathExtension.Clamp(z1, 0, h - 1);
            ...
            for (int z = z0; z <= z1; ++z)
            {
                ...
                x0 = MathExtension.Clamp(x0, -1, w - 1);
                x1 = MathExtension.Clamp(x1, 0, w - 1);
                ...
                for (int x = x0; x <= x1; ++x)
                {
                    ...
                    int spanMinCellIndex = MathExtension.Clamp((int)Math.Floor(spanMin * inverseCellHeight), 0, RC_SPAN_MAX_HEIGHT);
                    int spanMaxCellIndex = MathExtension.Clamp((int)Math.Ceiling(spanMax * inverseCellHeight), spanMinCellIndex + 1, RC_SPAN_MAX_HEIGHT);
