...
namespace DotRecast.Recast
{
    ...
    public static class RcMeshDetails
    {
            ...
            RcHeightPatch hp)
        {
            ...
            ix = MathExtension.Clamp(ix - hp.xmin, 0, hp.width - 1);
            iz = MathExtension.Clamp(iz - hp.ymin, 0, hp.height - 1);
            ...
            if (h == RC_UNSET_HEIGHT)
            {
                ...
                for (int i = 0; i < maxIter; ++i)
                {
                    ...
                    if (nx >= 0 && nz >= 0 && nx < hp.width && nz < hp.height)
                    {
                        ...
                        {
                            float d = (float)Math.Abs(nh * ch - fy);
                            if (d < dmin)
                            {
                                h = nh;
                                dmin = d;
            ...
            int[] verts, int bs, RcHeightPatch hp, List<int> array)
        {
            ...
            int[] dirs = { 0, 1, 2, 3 };
            ArrayExtension.Fill(hp.data, 0, 0, (hp.width * hp.height) - (0));
            ...
            array.Add(cy + bs);
            array.Add(ci);
            ArrayExtension.Fill(hp.data, RC_UNSET_HEIGHT, 0, (hp.width * hp.height) - (0));
