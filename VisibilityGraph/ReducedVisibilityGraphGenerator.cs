...
public class ReducedVisibilityGraphGenerator : MonoBehaviour
{
    ...
    public static List<Vector3d> vertices = new List<Vector3d>(); // The list of vertices in the graph
    public static List<Vector3d[]> edges = new List<Vector3d[]>(); // The list of edges in the graph
    ...
    private void VerticesInit()
    {
        ...
        // A helper method that converts (x, z) grid coordinates to a Vector3d Unity position and adds it to the list of vertices
        void AddCoordinatesToList(int x, int z)
        {
            ...
            Vector3d vertex = new Vector3d(coordinates[0], VERTEX_Y, coordinates[1]); // Create the corresponding Vector3d
    ...
    private void EdgesInit()
    {
        for (int i = 0; i < vertices.Count; i++)
        {
            ...
            {
                Vector3d[] edge = new Vector3d[] { vertices[i], vertices[j] }; // The current edge
     ...
     */
    public static bool IsEdgeBitangent(Vector3d[] edge, float extension)
    {
        Vector3d direction = (edge[1] - edge[0]);
        direction.Unitize();// The direction from the first vertex to the second vertex of the edge
        float distance = (edge[1] - edge[0]).Length + extension; // The distance between the two vertices, with a slight extension
