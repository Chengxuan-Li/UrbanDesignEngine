...
namespace UrbanDesignEngine.Triangulation
{
    ...
    public static class AStarAlgorithm
    {
        ...
        public static List<Point3d> AStar(Point3d start, Point3d end, UndirectedGraph<VGVertex, VGEdge> graph)
        {
            var sDists = graph.Vertices.ToList().ConvertAll(v => v.Location.DistanceTo(start));
            var eDists = graph.Vertices.ToList().ConvertAll(v => v.Location.DistanceTo(end));

            int s = sDists.FindIndex(v => v == sDists.Min());
            int e = eDists.FindIndex(v => v == eDists.Min());

            var ids = AStar(s, e, graph.Vertices.ToList().ConvertAll(p => new Vector3d(p.Location)),
                graph.Edges.ToList().ConvertAll(e => new Vector3d[] {
                    new Vector3d(e.Source.Location),
                    new Vector3d(e.Target.Location)
                }
            ));
            return ids.ConvertAll(i => graph.Vertices.ToList()[i].Location);
        }
         ...
         */
        public static List<int> AStar(int startVertex, int destinationVertex, List<Vector3d> vertices, List<Vector3d[]> edges)
        {
            ...
            {
                Vector3d vector = vertices[v2] - vertices[v1]; // A vector from v1 to v2
                return (float)vector.Length; // Return the magnitude of the vector
            }
            ...
            List<int> FindNeighbors(int v)
            {
                ...
                foreach (Vector3d[] edge in edges)
                {
